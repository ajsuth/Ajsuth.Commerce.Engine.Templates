...
namespace $rootnamespace$
{
    ...
    public class $safeitemname$ : IInventoryItemCondition
    {
        ...
        /// The target item id.
        /// </value>
        public IRuleValue<string> Location { get; set; }
        ...
        public bool Evaluate(IRuleExecutionContext context)
        {
            ...
            var location = this.Location.Yield(context);
            if (string.IsNullOrWhiteSpace(location))
            {
                commerceContext.Logger.LogError($"{nameof($safeitemname$)}: Location cannot be null or whitespace.");
                return false;
            }
            ...
            var inventoryInformation = Task.Run(() =>
                    Commander.Command<GetInventoryInformationCommand>().Process(commerceContext,
                        $"{CommerceEntity.IdPrefix<InventorySet>()}{location}",
                        sellableItem.Id,
